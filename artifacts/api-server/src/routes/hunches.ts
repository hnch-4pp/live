...
router.get("/hunches/:id/all-predictions", async (req, res): Promise<void> => {
  ...
  // How many questions does this hunch have? That's how many prediction rows = 1 participation.
  const [qCountRow] = await db
    .select({ cnt: count() })
    .from(hunchQuestionsTable)
    .where(eq(hunchQuestionsTable.hunchId, hunch.id));
  const questionsPerParticipation = Math.max(Number(qCountRow?.cnt ?? 1), 1);
  ...
  // Collect rows per user (already in createdAt order)
  const userRows = new Map<number, { username: string; labels: (string | null)[]; firstAt: Date }>();
  for (const row of rows) {
    if (row.userId === null) continue;
    if (!userRows.has(row.userId)) {
      userRows.set(row.userId, { username: row.username ?? "Anónimo", labels: [], firstAt: row.createdAt });
    }
    userRows.get(row.userId)!.labels.push(row.optionLabel);
  }

  // Split each user's labels into batches of questionsPerParticipation
  const predictions: { username: string; answer: string; submittedAt: string }[] = [];
  for (const [, { username, labels, firstAt }] of userRows) {
    const n = questionsPerParticipation;
    for (let i = 0; i < labels.length; i += n) {
      const batch = labels.slice(i, i + n);
      predictions.push({
        username,
        answer: batch.map((l) => l ?? "—").join(" / "),
        submittedAt: firstAt.toISOString(),
      });
    }
  }
